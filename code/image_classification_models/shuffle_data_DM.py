...
base_folder = '../MVTEC-AD-DepthMap'

# Paths to the new CSVs
new_train_csv = '../binary_labels/new_train_DM_colored.csv'
new_test_csv = '../binary_labels/new_test_DM_colored.csv'

# Paths to the new folders
new_train_folder = os.path.join(base_folder, 'new_train_DM_colored')
new_test_folder = os.path.join(base_folder, 'new_test_DM_colored')
...
def modify_image_names(df):
    df['image_path'] = df['image_path'].apply(lambda x: x.replace('.png', '_DM_colored.png'))
    return df
...
def copy_images(df, base_folder, new_folder):
    for idx, row in df.iterrows():
        ...
        img_name_DM = img_name.replace('.png', '_DM_colored.png') if not img_name.endswith('_DM_colored.png') else img_name
