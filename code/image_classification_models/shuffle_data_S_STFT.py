...
base_folder = '../MVTEC-AD-Spectrograms/STFT'

# Paths to the new CSVs
new_train_csv = '../binary_labels/new_train_S_STFT.csv'
new_test_csv = '../binary_labels/new_test_S_STFT.csv'

# Paths to the new folders
new_train_folder = os.path.join(base_folder, 'new_train_S_STFT')
new_test_folder = os.path.join(base_folder, 'new_test_S_STFT')
...
def modify_image_names(df):
    df['image_path'] = df['image_path'].apply(lambda x: x.replace('.png', '_stft.png'))
    return df
...
def copy_images(df, base_folder, new_folder):
    for idx, row in df.iterrows():
        ...
        img_name_DM = img_name.replace('.png', '_stft.png') if not img_name.endswith('_stft.png') else img_name
