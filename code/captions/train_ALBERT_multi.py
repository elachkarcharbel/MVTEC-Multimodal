...
import sentencepiece  # Make sure to import sentencepiece
...
def train_and_evaluate(folder, train_csv, test_csv):
    ...
    device = model.device
    class_weights = torch.tensor([1.0 / (train_df['label'].value_counts()[0]), 
                                  1.0 / (train_df['label'].value_counts()[1])], dtype=torch.float).to(device)
    model.classifier.weight.data = model.classifier.weight.data * class_weights.view(-1, 1)
